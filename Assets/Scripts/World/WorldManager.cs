...
public class WorldManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            WorldGeneration generator = GetComponent<WorldGeneration>();
            generator.GenerateGrid(new Vector2Int(5, 4));

            Vector2Int gridSize = new Vector2Int(80, 64); //size * 16

            for (int y = 0; y < 4; y++)
            {
                for (int x = 0; x < 5; x++)
                {
                    TextAsset textFile = Resources.Load<TextAsset>("WFCData/" + generator.grid[x,y].options[0].name);
                    string[] textLines = textFile.text.Split("\n");
                    
                    for (int l = textLines.Length - 1; l >= 0; l--)
                    {
                        for (int c = 0; c < textLines[l].Length; c++)
                        {
                            char ltr = textLines[l][c];
                            if (ltr == '0')
                            {
                                tilemaps[0].SetTile(new Vector3Int(x * 16 + c - gridSize.x / 2, y * 16 + (15 - l) - gridSize.y / 2, 0), allTiles[(int)TileType.Cliff]);
                                //tilemaps[1].SetTile(new Vector3Int(x * 16 + c - gridSize.x / 2, y * 16 + l - gridSize.y / 2, 0), allTiles[(int)TileType.Water]);
                            }
                            else
                            {
                                tilemaps[0].SetTile(new Vector3Int(x * 16 + c - gridSize.x / 2, y * 16 + (15 - l) - gridSize.y / 2, 0), allTiles[(int)TileType.Grass]);
                                tilemaps[1].SetTile(new Vector3Int(x * 16 + c - gridSize.x / 2, y * 16 + (15 - l) - gridSize.y / 2, 0), null);
                            }
                        }
                    }
                }
    ...
    private void LoadMap(SaveData saveData)
    {
        ...
        {
            Structure newHouse = Resources.Load<Structure>($"Prefabs/{child.prefabName}");
